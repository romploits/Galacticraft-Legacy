...
public class TabRegistry
{
    ...
    static
    {
        try
        {
            // Checks for JEI by looking for this class instead of a
            // Loader.isModLoaded() check
    ...
    public static int getPotionOffset()
    {
         ...
         * is active... if (doPotionOffsetVanilla()) { initWithPotion = true;
        ...
        if (clazzJEIConfig != null)
        {
            try
            {
                ...
                if (enabled instanceof Boolean)
                {
                    if (!((Boolean) enabled))
                    {
                        // If JEI is disabled, no special change to
                        // getPotionOffset()
                        return 0;
                    }
                    // Active JEI undoes the standard potion offset (they listen
                    // for GuiScreenEvent.PotionShiftEvent)
                    return -60;
                }
            } catch (Exception ignore)
            {
        ...
        if (initWithPotion && clazzNEIConfig != null)
        {
            try
            {
                ...
                if (hidden instanceof Boolean && enabled instanceof Boolean)
                {
                    ...
                    {
                        // If NEI is disabled or hidden, it does not affect the
                        // tabs offset with potions
                        return 0;
                    }
