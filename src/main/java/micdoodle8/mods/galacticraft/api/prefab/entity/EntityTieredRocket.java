...
//        this.yOffset = this.height / 2.0F;
    ...
    private void initiatePlanetsPreGen(int cx, int cz)
    {
        ...
        // Pre-generate terrain on all possible destination planets if the
        // destination is not being controlled by a Launch Controller
        // (note: this does NOT include the Moon!)

        // This generates with a chunk radius of 12: so for 2 planets that's
        // 1250 chunks to pregen
        // It starts at the centre and generates in circles radiating out in
        // case it doesn't have time to finish
        // These will be done: 2 chunks per tick during IGNITE phase (so 800
        // chunks during the 20 second launch countdown)
        // then the ones that are left 1 chunk per tick during flight (normally
        // flight will last more than 450 ticks)
        // If the server is at less than 20tps then maybe some of the outermost
        // chunks won't be pre-generated but that's probably OK
        if (this.destinationFrequency == -1 && !EntityTieredRocket.preGenInProgress)
        {
            ...
            if (toPreGen.size() > 0)
            {
                ...
                for (int r = 1; r < 12; r++) // concentric squares with radius r
                {
                    ...
                    int zmin = cz - r;
                    int zmax = cz + r;
                    for (int i = -r; i < r; i++) // stop before i == r to avoid
                                                 // doing corners twice
                    {
                        for (Integer dimID : toPreGen)
                        {
                ...
                if (this.preGenIterator.hasNext())
                {
                    ...
                    if (this.world instanceof WorldServer)
                    {
                        ...
                        if (w != null)
                        {
                            ...
                            // Pregen a second chunk if still on launchpad (low
                            // strain on server)
    ...
    public void onReachAtmosphere()
    {
        ...
        if (this.destinationFrequency != -1)
        {
            if (this.world.isRemote)
            {
                // stop the sounds on the client - but do not reset, the rocket
                // may start again
            ...
            if (this.targetVec != null)
            {
                if (this.targetDimension != this.world.provider.getDimension())
                {
                    ...
                    // No destination world found - in this situation continue
                    // into regular take-off (as if Not launch controlled)
                } else
                {
                    ...
                    // Stop any lateral motion, otherwise it will update to an
                    // incorrect x,z position first tick after spawning above
                    // target
                    this.motionX = this.motionZ = 0.0D;
                    // Small upward motion initially, to keep clear of own flame
                    // trail from launch
                    this.motionY = 0.1D;
                    for (Entity passenger : this.getPassengers())
                    {
        ...
        if (!this.world.isRemote)
        {
            ...
            // Destroy any rocket which reached the top of the atmosphere and is
            // not controlled by a Launch Controller
            this.setDead();
        }

        // Client side, non-launch controlled, do nothing - no reason why it
        // can't continue flying until the GUICelestialSelection activates
    }
    ...
    public boolean processInitialInteract(EntityPlayer player, EnumHand hand)
    {
        ...
        } else if (player instanceof EntityPlayerMP)
        {
            if (!this.world.isRemote)
            {
                GalacticraftCore.packetPipeline.sendTo(new PacketSimple(EnumSimplePacket.C_DISPLAY_ROCKET_CONTROLS, this.world.provider.getDimension(), new Object[]
                {}), (EntityPlayerMP) player);
                GCPlayerStats stats = GCPlayerStats.get(player);
                stats.setChatCooldown(0);
