...
public interface ITileClientUpdates
{
    ...
    default void clientOnLoad()
    {
        ...
        {
            GalacticraftCore.packetPipeline.sendToServer(new PacketSimple(EnumSimplePacket.S_REQUEST_MACHINE_DATA, tile.getWorld(), new Object[]
            {tile.getPos()}));
        }
    }

    /**
     * Do not override unless you want to use a custom data packet with more
    ...
    default void sendUpdateToClient(EntityPlayerMP player)
    {
        ...
        GalacticraftCore.packetPipeline.sendTo(new PacketSimple(EnumSimplePacket.C_UPDATE_MACHINE_DATA, GCCoreUtil.getDimensionID(player.world), new Object[]
        {((TileEntity) this).getPos(), data[0], data[1], data[2], data[3]}), player);
    }
    ...
    default void updateAllInDimension()
    {
        ...
        GalacticraftCore.packetPipeline.sendToDimension(new PacketSimple(EnumSimplePacket.C_UPDATE_MACHINE_DATA, dimID, new Object[]
        {((TileEntity) this).getPos(), data[0], data[1], data[2], data[3]}), dimID);
    }
}
