...
public class OverlayLander extends Overlay
{
    ...
    public static void renderLanderOverlay(Minecraft mc, long ticks)
    {
        ...
        {
            mc.fontRenderer.drawString(GCCoreUtil.translate("gui.warning"), width / 4 - mc.fontRenderer.getStringWidth(GCCoreUtil.translate("gui.warning")) / 2, height / 8 - 20,
            ...
            mc.fontRenderer.drawString(press1 + GameSettings.getKeyDisplayString(KeyHandlerClient.spaceKey.getKeyCode()) + press2,
                width / 4 - mc.fontRenderer.getStringWidth(press1 + GameSettings.getKeyDisplayString(KeyHandlerClient.spaceKey.getKeyCode()) + press2) / 2, height / 8,
        ...
        {
            String string = GCCoreUtil.translate("gui.lander.velocity") + ": " + Math.round(((EntityLander) mc.player.getRidingEntity()).motionY * 1000) / 100.0D + " "
                + GCCoreUtil.translate("gui.lander.velocityu");
            int color =
                ColorUtil.to32BitColor(255, (int) Math.floor(Math.abs(mc.player.getRidingEntity().motionY) * 51.0D), 255 - (int) Math.floor(Math.abs(mc.player.getRidingEntity().motionY) * 51.0D), 0);
