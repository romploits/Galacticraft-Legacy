...
public class NetworkUtil
{
    ...
    public static void encodeData(ByteBuf buffer, Collection<Object> sendData) throws IOException
    {
        for (Object dataValue : sendData)
        {
            ...
            } else
            {
                if (dataValue == null)
                {
                    GalacticraftCore.logger.error("Cannot construct PacketSimple with null data, this is a bug.");
                }
                GalacticraftCore.logger.info("Could not find data type to encode!: " + dataValue);
            }
        }
    }
            ...
            } else if (clazz.equals(Footprint[].class))
            {
                ...
                for (int i = 0; i < size; i++)
                {
                    objList.add(new Footprint(buffer.readInt(), new Vector3(buffer.readFloat(), buffer.readFloat(), buffer.readFloat()), buffer.readFloat(), buffer.readShort(),
                        ByteBufUtils.readUTF8String(buffer), -1));
                }
    ...
    public static boolean fuzzyEquals(Object a, Object b)
    {
        ...
        } else if (a instanceof EnergyStorage && b instanceof EnergyStorage)
        {
            ...
            return fuzzyEquals(a2.getEnergyStoredGC(), b2.getEnergyStoredGC()) && fuzzyEquals(a2.getCapacityGC(), b2.getCapacityGC()) && fuzzyEquals(a2.getMaxReceive(), b2.getMaxReceive())
        ...
        } else if (a instanceof FluidTank && b instanceof FluidTank)
        {
            ...
            return fuzzyEquals(a2.getCapacity(), b2.getCapacity()) && fuzzyEquals(fluidA != null ? fluidA.getFluid().getName() : "", fluidB != null ? fluidB.getFluid().getName() : "")
