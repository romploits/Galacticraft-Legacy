    ...
    public void update()
    {
        if (!this.world.isRemote && this.corner == 0)
        {
            ...
            if (success)
            {
                ...
                // Don't try setting a new block with a TileEntity, because new
                // tiles can
                // get removed after the end of this tileEntity.update() tick -
                // setting a new block
                // here would automatically invalidate this tile.
                //
                // (It's because if this tileEntity is now invalid,
                // World.updateEntities() removes
                // *any* tileEntity at this position - see call to
                // Chunk.removeTileEntity(pos)nee!)
                //
                // Equally if any others of these TileEntityMinerBaseSingle are
                // ticked AFTER this
                // in the same server update tick, then those new ones will also
                // be removed
                // because their TileEntityMinerBaseSingle is now, inevitably,
                // invalid!
            }
        }
    }
