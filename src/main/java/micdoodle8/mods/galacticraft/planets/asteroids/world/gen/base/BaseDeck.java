...
public class BaseDeck extends SizedPiece
{
    ...
    protected int deckTier; // flag 1 means lowest deck in a stack, flag 2 is
                            // for highest deck, 0 is for middle decks - flag 4
                            // is for control room
    ...
    public BaseDeck(BaseConfiguration configuration, Random rand, int blockPosX, int blockPosY, int blockPosZ, int tier, EnumFacing dir)
    {
        ...
        if (tier == 1) // 1 means lowest tier and there are going to be some
                       // higher ones
        {
    ...
    public boolean addComponentParts(World worldIn, Random randomIn, StructureBoundingBox chunkBounds)
    {
            ...
            for (int z = startZ; z <= endZ; z++)
            {
                for (int y = 0; y <= endY; y++)
                {
                    if (x == startX || x == endX || y == 0 || y == endY || z == startZ || z == endZ)
                    {
                        ...
                        if (this.configuration.isHangarDeck() && this.deckTier == 1 && (this.getDirection() == EnumFacing.NORTH && z == endZ || this.getDirection() == EnumFacing.SOUTH && z == startZ
                            || this.getDirection() == EnumFacing.WEST && x == endX || this.getDirection() == EnumFacing.EAST && x == startX))
                        {
                        ...
                        // Skip certain blocks at top and bottom to give
                        // chamfered edge
                        else if (!((y == 0 && (this.deckTier & 1) == 1 || y == endY && (this.deckTier & 1) == 2)
                            && (directionNS && (z == startZ || z == endZ) || !directionNS && (x == startX || x == endX))))
                        {
                            if (y == 0 && (this.deckTier & 1) == 1 && this.configuration.getDeckType() == EnumBaseType.HUMANOID
                                && (directionNS && (x == startX + 2 + (this.configuration.isHangarDeck() ? 1 : 0) || x == startX + 4 + (this.configuration.isHangarDeck() ? 1 : 0))
                                    || !directionNS && (z == startZ + 2 + (this.configuration.isHangarDeck() ? 1 : 0) || z == startZ + 4 + (this.configuration.isHangarDeck() ? 1 : 0))))
                            {
                                this.setBlockState(worldIn, blockGrid, x, y, z, chunkBounds);
                            } else if (!this.configuration.isHangarDeck() && (this.getDirection() == EnumFacing.SOUTH && z == endZ || this.getDirection() == EnumFacing.NORTH && z == startZ
                                || this.getDirection() == EnumFacing.EAST && x == endX || this.getDirection() == EnumFacing.WEST && x == startX))
                            {
                        ...
                        if (y == 0 && (this.deckTier & 1) == 1 && this.configuration.getDeckType() == EnumBaseType.HUMANOID
                            && (directionNS && (x >= startX + 2 + hangarOffset && x <= startX + 4 + hangarOffset)
                                || !directionNS && (z >= startZ + 2 + hangarOffset && z <= startZ + 4 + hangarOffset)))
                        {
        ...
        {
            this.setCoordBaseMode(this.direction.getOpposite()); // bug in
                                                                 // Vanilla?
        }
    ...
    public List<Piece> getRooms(int roomIndex, BaseStart startPiece, Random rand)
    {
        ...
        // now offset effect of moving control room 1 position North or West, in
        // two cases
...
//        System.out.println("Making doorway at " + x + "," + z + " NS:" + directionNS + " Tier " + this.deckTier);
    ...
    protected Piece getRoom(int i, EnumFacing dir, int blockX, int blockZ, boolean large, boolean left, Random rand)
    {
        ...
        int sY = this.sizeY;
        return new BaseRoom(this.configuration, rand, blockX, this.boundingBox.minY, blockZ, sX, sY, sZ, dir, this.configuration.getRandomRoom(i), left ? (i == 0) : (i == this.roomsOnSide - 1),
            left ? (i == this.roomsOnSide - 1) : (i == 0), this.deckTier);
    }
}
