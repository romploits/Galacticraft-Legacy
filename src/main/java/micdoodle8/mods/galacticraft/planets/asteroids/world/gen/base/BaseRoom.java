...
public class BaseRoom extends SizedPiece
{
            ...
            for (int yy = 0; yy <= this.sizeY; yy++)
            {
                for (int zz = 0; zz <= maxZ; zz++)
                {
                    ...
                    if (xx == 0 || xx == maxX || yy == 0 || yy == this.sizeY || zz == maxZ)
                    {
                        ...
                        else if (!((zz == maxZ || near || far) && (yy == 0 && (this.deckTier & 1) == 1 || yy == this.sizeY && (this.deckTier & 2) == 2 || (zz == maxZ && (near || far))))
                            || zz == 0 && yy == 0)
                        {
                            this.setBlockState(worldIn, this.configuration.getWallBlock(), xx, yy, zz, chunkBoundary);
                        }
                        // Special case, fill in some corners on hangardeck top
                        // deck
    ...
    /**
     * Room contents boundaries are: x from 1 to maxX y from 1 to this.sizeY - 1
     * z from 1 to maxZ
     */
    private void buildRoomContents(World worldIn, int x, int y, int z, int maxX, int maxZ, BlockPos blockpos, int randomInt)
    {
        ...
        switch (this.type)
        {
            ...
            case STORE:
                if (maxX >= 6 && y == 1 && maxZ >= 5)
                {
                    ...
                    {
                        // Cargo loaders and unloaders, these will contain
                        // treasure items but only accessible when powered
                        switch (z)
                        {
                            case 2:
            ...
            case POWER:
                ...
                {
                    case 1:
                        // Layer 1: wall blocks with sealable alu wire in the
                        // centre
            ...
            case MEDICAL:
                ...
                } else if (y == 2 || y == 3)
                {
                    ...
                    {
                        // Fluid tanks, 2 tanks high - these will be filled with
                        // Bacterial Sludge below
                        if ((maxX - x) % 2 == 1)
                        {
            ...
            case CRYO:
                ...
                {
                    // Around the walls: Cryo Chambers alternated with dark
                    // blocks
        ...
        if (state.getBlock() instanceof ITileEntityProvider)
        {
            ...
            } else if (tile instanceof TileEntityCargoLoader)
            {
                ...
                loader.addCargo(new ItemStack(MarsItems.marsItemBasic, semirand % 2 + 1, 4), true); // Slimeling
                                                                                                    // Inventory
                                                                                                    // Bag
                loader.addCargo(new ItemStack(AsteroidsItems.basicItem, semirand % 23 + 41, 7), true); // Thermal
                                                                                                       // cloth
        ...
        if (this.type == EnumRoomType.ENGINEERING && y == (this.configuration.getRoomHeight() <= 4 ? 2 : 3) && z == maxZ && (x == 3 || x == 6))
        {
            ...
            {
                // Apparently we have our North and our South reversed ? So we
                // don't want the opposite for North and South!
