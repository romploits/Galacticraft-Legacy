...
public class GuiLaserTurretEditList extends GuiScreen implements GuiElementTextBox.ITextBoxCallback, GuiElementCheckbox.ICheckBoxCallback
{
            ...
            case 2:
                ...
                if (selected != null)
                {
                    ...
                    GalacticraftCore.packetPipeline
                        .sendToServer(new PacketSimpleVenus(PacketSimpleVenus.EnumSimplePacketVenus.S_MODIFY_LASER_TARGET, GCCoreUtil.getDimensionID(laserTurret.getWorld()), new Object[]
                        {isPlayer ? 2 : 3, laserTurret.getPos(), toSend}));
                    if (isPlayer)
                    {
            ...
            case 4:
                if (mode == EnumMode.ADD_ENTITY)
                {
                    ...
                    if (entry != null)
                    {
                        GalacticraftCore.packetPipeline
                            .sendToServer(new PacketSimpleVenus(PacketSimpleVenus.EnumSimplePacketVenus.S_MODIFY_LASER_TARGET, GCCoreUtil.getDimensionID(laserTurret.getWorld()), new Object[]
                            {1, laserTurret.getPos(), entry.getRegistryName().toString()}));
                ...
                } else if (mode == EnumMode.ADD_PLAYER)
                {
                    if (name.text != null && !name.text.isEmpty())
                    {
                        GalacticraftCore.packetPipeline
                            .sendToServer(new PacketSimpleVenus(PacketSimpleVenus.EnumSimplePacketVenus.S_MODIFY_LASER_TARGET, GCCoreUtil.getDimensionID(laserTurret.getWorld()), new Object[]
                            {0, laserTurret.getPos(), name.text}));
    ...
    public void initGui()
    {
        ...
        switch (mode)
        {
            ...
            case ADD_ENTITY:
                ...
                this.buttonList.add(new GuiButton(4, (this.width - this.xSize) / 2 + this.xSize / 2 - 31, yTop + 40, 62, 20,
                    GCCoreUtil.translate(laserTurret.blacklistMode ? "gui.button.add_blacklist.name" : "gui.button.add_whitelist.name")));
                break;
        }
    }
    ...
    public void onSelectionChanged(GuiElementCheckbox checkbox, boolean newSelected)
    {
        ...
        {2, this.laserTurret.getPos(), this.laserTurret.alwaysIgnoreSpaceRace ? 1 : 0}));
    }
